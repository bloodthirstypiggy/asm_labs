...
build: main.s
	$(AS) -g -o $(BASENAME).o $^
	$(LD) -L /usr/aarch64-linux-gnu/ -lc -lm -g -o $(BASENAME).out $(BASENAME).o 
	-@rm $(BASENAME).o
...
.PHONY: run
run: $(BASENAME).out
	qemu-aarch64 -L /usr/aarch64-linux-gnu $(BASENAME).out makerun
...
debug: $(BASENAME).out
	...
	tmux new-session -d 'qemu-aarch64 -L /usr/aarch64-linux-gnu -g 31337 $(BASENAME).out debug; $$SHELL'
